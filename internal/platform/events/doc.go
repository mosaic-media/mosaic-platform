...
// worker that drains committed outbox rows into it.
//
...
// the other half — it polls contracts.EventOutbox.ListUnpublished, publishes
// each deliverable event through an EventPublisher, and on success marks it
// published or on failure records the failure so the Platform delivery policy
// can schedule a retry or dead-letter the event.
//
