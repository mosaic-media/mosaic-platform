...
- **Every Go file carries an SPDX header** (`AGPL-3.0-only`, the Platform's license). New files get it from the tool, not by hand: `go run ./tools/licenseheader` adds it to any file missing it. **CI enforces it** — `.github/workflows/verify.yml` runs `go run ./tools/licenseheader -check` (plus gofmt/vet/build) on every push and PR, so a headerless file fails the check. Change the header text in one place — the `header` const in that tool.
